...
def spectrum(age, logZ, pagb):
    ...
    sp_dict = dict(zcontinuous=1,
                   ...
                   const=0.0,
                   sf_start=0.0)
    spec = fsps.StellarPopulation(**sp_dict)
    spec.params['logzsol'] = logZ
    spec.params['pagb'] = pagb
    print(c, age, logZ, pagb)
    s = spec.get_spectrum(tage=age/1.e9)[1]
    print(c, age, logZ, pagb, s)
    return age, logZ, pagb, s   # lsun per hz
...
def hden_ascii(fileout, **kwargs):
    ...
    dat = po.map(multi_run_wrapper, modpars)
    for row in dat:
        a.append(row[0])
        z.append(row[1])
        p.append(row[2])
    ...
    for age in ages:
        for logZ in logZs:
            for pagb in pagbs:
                c += 1
                print(c, age, logZ, pagb)
                all_fluxs.append(sp[(np.where((a == age) & (z == logZ) & (p == pagb)))][0])
    nmod = len(modpars)
...
mod_dir = '/ufrc/narayanan/prerakgarg/cloudy_runs/pagb/'
mod_prefix = 'ZAU'
...
ages = np.array([1, 2, 3, 4, 5, 6, 7, 10, 500, 1000, 2000, 3000, 4000, 5000, 6000, 7000, 8000, 9000, 10000])*1.e6
# stellar metallicities
logZs = np.array([-2, -1.5, -1.0, -0.6, -0.4, -0.3, -0.2, -0.1, 0.0, 0.1, 0.2])
...
# stellar metallicities
logZs = np.array([-1.98, -0.5])
# ionization parameters between -4 and -1
logUs = np.array([-5.0, -0.5])
# Hydrogen density between 30 and 400
#nhs = np.arange(50., 450., 200) # density of gas, cm-3
nhs = np.array([10, 100])
# Post-AGB stars weight in FSPS
Pagbs = np.array([0.001, 1])
